...
public class MetodosCalculoEmisionImp extends MetodosCalculoEmision {
    ...
    public MetodosCalculoEmisionImp(double Volumen, double Capacidad, double CargaMinima, double PorRuta, double KilometrosDesviar) {
    ...
    public boolean InicioProceso() {
        ...
        try {
            for (int i = 0; i < datos.getPesoVolProvedores().size(); i++) {
                ...
                ajustarDistanciasEmision();
                datos.getDistanciasProvicional().put(dias, datos.getDistanciasEmision().get(dias));
                System.out.println("Ruta para " + dias);
                System.out.println("");
                CrearPuntosArrayList(datos.getDistanciasEmision().get(dias), dias, datos.getPesoVolProvedores().size()); // unir parejas con ahorro
...
//                    }
    ...
    public ArrayList<ArrayList<Double>> CrearArreglo(ArrayList<ArrayList<Double>> nodosAhorro, ArrayList<ArrayList<Double>> superanCapacidad, ArrayList<ArrayList<Double>> nodosDirectos, String dia) {
        ...
        for (int i = 0; i < nodosDirectos.size(); i++) {
            ...
            if (nodosDirectos.get(i).get(1) != 0 && nodosDirectos.get(i).get(2) != 0) {
                if (nodosDirectos.get(i).get(0) < datos.getDistanciasEmision().get(dias).length) {
    ...
    private void ajustarDistanciasEmision() {
        ArrayList<ArrayList<Double>> tmpCopia = new ArrayList<>();
        for (int y = 0; y < datos.getDistancias().length; y++) {
            tmpCopia.add(new ArrayList<>());
            for (int z = 0; z < datos.getDistancias()[y].length; z++) {
                if (datos.getDistancias()[y].length == 1) {
                    tmpCopia.get(y).add(datos.getDistancias()[y][0]);
                    break;
                } else {
                    tmpCopia.get(y).add(datos.getDistancias()[y][z]);
                }
            }
        }
        for (int i = 0; i < datos.getPesoVolProvedores().get(dias).get(Constantes.Constantes.Demanda_kg).size(); i++) {
            double multiplicador = SacarMultiplicador(datos.getPesoVolProvedores().get(dias).get(Constantes.Constantes.Demanda_kg).get(i));
            for (int z = 0; z < tmpCopia.get(i).size(); z++) {
                tmpCopia.get(i).set(z, tmpCopia.get(i).get(z) * multiplicador);
            }
        }
        double[][] rutas = new double[tmpCopia.size()][];
        for (int p = 0; p < tmpCopia.size(); p++) {
            rutas[p] = new double[tmpCopia.get(p).size()];
            for (int t = 0; t < tmpCopia.get(p).size(); t++) {
                rutas[p][t] = tmpCopia.get(p).get(t);
        ...
        datos.getDistanciasEmision().put(dias, rutas);
    }

    private double SacarMultiplicador(double num) {
        if (num >= Double.parseDouble(Constantes.Constantes.TablaConversionPeso0) && num <= Double.parseDouble(Constantes.Constantes.TablaConversionPeso5999)) {
            return Double.parseDouble(Constantes.Constantes.TablaMultiplicador1);
        } else if (num >= Double.parseDouble(Constantes.Constantes.TablaConversionPeso6000) && num <= Double.parseDouble(Constantes.Constantes.TablaConversionPeso11999)) {
            return Double.parseDouble(Constantes.Constantes.TablaMultiplicador2);
        } else if (num >= Double.parseDouble(Constantes.Constantes.TablaConversionPeso12000) && num <= Double.parseDouble(Constantes.Constantes.TablaConversionPeso17999)) {
            return Double.parseDouble(Constantes.Constantes.TablaMultiplicador3);
        } else if (num >= Double.parseDouble(Constantes.Constantes.TablaConversionPeso18000) && num <= Double.parseDouble(Constantes.Constantes.TablaConversionPeso20999)) {
            return Double.parseDouble(Constantes.Constantes.TablaMultiplicador4);
        } else if (num >= Double.parseDouble(Constantes.Constantes.TablaConversionPeso21000) && num <= Double.parseDouble(Constantes.Constantes.TablaConversionPeso24000)) {
            return Double.parseDouble(Constantes.Constantes.TablaMultiplicador5);
        }
        return 0D;
    }
}
